...
// The live previews render the patient app's real card and real formatters,
// so the CMS can never drift from what ships.
import '../../../patient/screens/widgets/care_service_card.dart';
import '../../../patient/screens/widgets/dynamic_home_sections.dart';
...
///
/// Split out of `AdminHomeSectionsPage`, which is now the Care Services layout
/// picker alone. The two share a backing collection but not a job: this page
/// edits the rows *below* Care Services, while that one only decides how the
/// Care Services block itself is arranged.
class AdminDynamicSectionsPage extends ConsumerWidget {
  const AdminDynamicSectionsPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // CARE_SERVICES is reserved: it is the Care Services block on Home, not a
    // row in this list. Its one editable property — the layout — is owned by
    // AdminHomeSectionsPage, so showing it here as an ordinary section would
    // offer an editor for content it doesn't have.
    final async = ref
        .watch(allHomeSectionsProvider)
        .whenData((items) => [for (final s in items) if (!s.isCareServices) s]);
          ...
          children: [
            ...
            PopupMenuButton<String>(
              ...
              onSelected: (action) async {
                if (action == 'edit') {
                  AdminDynamicSectionsPage._openForm(context, ref,
                      existing: section);
...
class _DraftItem {
  ...
  final TextEditingController subtitleBn;
  final TextEditingController priceTag;
  final TextEditingController badgeText;
  /// Per-card visibility — hides this card on Home without deleting it.
  bool isActive;
  /// Which Home filter pill this card hides behind. Null ⇒ untagged, so it
  /// only appears under the implicit "All" pill.
  String? categoryId;
  ...
  _DraftItem({
    ...
    HomeSectionItem? from,
        ...
        titleBn = TextEditingController(text: from?.titleBn ?? ''),
        subtitleBn = TextEditingController(text: from?.subtitleBn ?? ''),
        priceTag = TextEditingController(text: from?.priceTag ?? ''),
        badgeText = TextEditingController(text: from?.badgeText ?? ''),
...
class _ItemCard extends StatelessWidget {
  ...
  /// Rebuilds the dialog after an edit that lives on the draft rather than in
  /// a form field — the target dropdown / service pick, the visibility switch,
  /// and every keystroke that the live preview has to reflect.
        ...
        children: [
          Row(
            children: [
              ...
              // Per-card visibility. Hiding drops the card from the patient
              // feed while leaving it here, fully editable, to switch back on.
              Switch(
              ...
              Text(
                ...
                style: MtTextStyles.bodySm.copyWith(
                  color: draft.isActive ? MtColors.completed : MtColors.ink3,
                ),
              ),
              const Spacer(),
              IconButton(
              ...
              Expanded(
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 8),
                    _Field(
                      controller: draft.subtitleBn,
                      hint: 'Subtitle (Bengali)',
                      onChanged: (_) => onChanged(),
                    ),
                  ],
                ),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _Field(
                  controller: draft.badgeText,
                  hint: 'Badge, e.g. Popular',
                  onChanged: (_) => onChanged(),
                ),
              ),
            ],
          ...
          const SizedBox(height: 12),
          _ItemCategoryPicker(draft: draft, onChanged: onChanged),
          const SizedBox(height: 12),
          _ItemCardPreview(draft: draft),
          const SizedBox(height: 12),
          _ItemTargetSection(draft: draft, onChanged: onChanged),
          const SizedBox(height: 4),
...
/// Which Home filter pill this card hides behind.
///
/// Only meaningful in the Care Services block, where the pill rail filters the
/// cards — the field is on every item because CARE_SERVICES is an ordinary
/// section with a reserved key, not a separate schema. "No category" is the
/// default and a legitimate answer: the card then shows only under "All".
class _ItemCategoryPicker extends ConsumerWidget {
  final _DraftItem draft;
  final VoidCallback onChanged;

  const _ItemCategoryPicker({required this.draft, required this.onChanged});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final categories = ref.watch(allCategoriesProvider).maybeWhen(
          data: (items) => [for (final c in items) if (c.isActive) c],
          orElse: () => const <HomeCategory>[],
        );
    // A tag pointing at a category that has since been hidden or deleted would
    // otherwise silently reset to "No category" on the next save. Show the
    // stored value as an explicit dangling entry instead.
    final ids = categories.map((c) => c.id).toSet();
    final dangling = draft.categoryId != null && !ids.contains(draft.categoryId)
        ? draft.categoryId
        : null;
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String?>(
          ...
          items: [
            const DropdownMenuItem<String?>(
              value: null,
              child: Text('No category — shows under "All" only'),
            ),
            for (final c in categories)
              DropdownMenuItem<String?>(
                value: c.id,
                child: Text('${c.nameEn}  ·  ${c.slug}'),
              ),
            if (dangling != null)
              DropdownMenuItem<String?>(
                value: dangling,
                child: const Text('(deleted or hidden category)'),
              ),
          ],
          onChanged: (v) {
            draft.categoryId = v;
        ...
        if (categories.isEmpty) ...[
          ...
          Text(
            'No categories yet — add them under CMS → Home categories.',
            style: MtTextStyles.bodySm.copyWith(color: MtColors.ink3),
          ),
        ],
      ],
...
/// Live preview of how this card will look on the patient Home.
///
/// Renders the **actual** [CareServiceCard] the app renders, fed through the
/// same [dynamicCardTitle] / [dynamicCardDescription] / [dynamicPriceLine]
/// helpers the patient rail uses — a hand-drawn mock would start lying the
/// first time either side changed. The card is wrapped in a dark [Theme] because
/// its palette resolves off `Theme.of(context).brightness`, and the patient Home
/// is a dark surface while the admin console is light.
///
/// Preview only: taps are swallowed rather than dispatched.
class _ItemCardPreview extends StatelessWidget {
  final _DraftItem draft;

  const _ItemCardPreview({required this.draft});
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
            // Approximates the Home canvas so the card is judged against the
          ...
          child: Align(
            ...
            child: SizedBox(
              ...
              child: Opacity(
                // A hidden card previews dimmed — it still needs to be
                // editable, but it must not read as live.
                opacity: draft.isActive ? 1 : 0.45,
                child: Theme(
...
class _Field extends StatelessWidget {
  ...
  /// Fires per keystroke — used by the item editor to repaint its live preview.
